...
public class SecurityCommand implements SimpleCommand {
        ...
        switch (args[0].toLowerCase()) {
            case "blockserverips":
                ...
                if (args[1].equalsIgnoreCase("on") || args[1].equalsIgnoreCase("true")) {
                    ...
                    invocation.source().sendMessage(Component.text("Usage: /security blockserverips <on|off>", NamedTextColor.RED));
                }
                break;
            ...
            case "notifyadmins":
                ...
                if (args[1].equalsIgnoreCase("on") || args[1].equalsIgnoreCase("true")) {
                    ...
                    invocation.source().sendMessage(Component.text("Usage: /security notify <on|off>", NamedTextColor.RED));
                }
                break;
    ...
    public List<String> suggest(Invocation invocation) {
        ...
        if (args.length == 0 || args.length == 1) {
            suggestions.add("blockserverips");
            suggestions.add("notify");
            suggestions.add("status");
        } else if (args.length == 2) {
            if (args[0].equalsIgnoreCase("blockserverips") || args[0].equalsIgnoreCase("notify")) {
                suggestions.add("on");
                suggestions.add("off");
            }
        }
